...
±±³Fun‡…o    ³ PCPR004  ³ Autor ³ Felipe Nunes Toledo   ³ Data ³ 26/09/06 ³±±
...
Static Function ReportDef()
	...
	//³                                                                        ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	oReport:= TReport():New("PCPR004",cTitle,"MTR820", {|oReport| ReportPrint(oReport, cAliasTop)},OemToAnsi(STR0001)) //"Este programa ira imprimir a Rela‡„o das Ordens de Produ‡„o"
	...
	// (cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD)
	TRCell():New(oSection1,'OP'			,'SC2',"OP"   , ,TamSX3('C2_NUM')[1]+TamSX3('C2_ITEM')[1]+TamSX3('C2_SEQUEN')[1]+TamSX3('C2_ITEMGRD')[1]	  ,/*lPixel*/,{|| (cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD) })
	//TRCell():New(oSection1,'C2_NUM'		,'SC2',"Código"   ,PesqPict('SC2','C2_NUM')	 ,TamSX3('C2_NUM')[1]	  ,/*lPixel*/,/*{|| code-block de impressao }*/)
	TRCell():New(oSection1,'C2_PRODUTO'	,'SC2',/*Titulo*/,/*Picture*/					,/*Tamanho*/			,/*lPixel*/,/*{|| code-block de impressao }*/)
	TRCell():New(oSection1,'B1_DESC' 	,'SB1',/*Titulo*/,/*Picture*/					,		,/*lPixel*/,/*{|| code-block de impressao }*/)
	TRCell():New(oSection1,'Emissao'   	,'SC2',"Emis."   ,PesqPict("SC2","C2_EMISSAO")	,10						,/*lPixel*/,{|| DTOC(dDataBase) })
	...
	TRCell():New(oSection1,'QtdeProd'	,'SC2',"Qtd."   ,PesqPict("SC2","C2_QUANT")	,TamSX3("C2_QUANT")[1]	,/*lPixel*/,{|| aSC2Sld(cAliasTop) })
	TRCell():New(oSection1,'C2_QUANT'	,'SC2',"Qtd."   ,/*Picture*/					,/*Tamanho*/			,/*lPixel*/,/*{|| code-block de impressao }*/)
	TRCell():New(oSection1,'OpQuant'  	,'SC2',"Qtd."   ,PesqPict("SC2","C2_QUANT")	,TamSX3("C2_QUANT")[1]	,/*lPixel*/,{|| (cAliasTop)->C2_QUANT - (cAliasTop)->C2_QUJE })
	TRCell():New(oSection1,'B1_UM'	 	,'SB1',"Un.",/*Picture*/					,/*Tamanho*/			,/*lPixel*/,/*{|| code-block de impressao }*/)
	...
	TRCell():New(oSection2,'D4_COD'	 	,'SD4',STR0058   ,PesqPict('SD4','D4_COD')    ,8    ,/*lPixel*/,/*{|| code-block de impressao }*/,/*cAlign*/,/*lLineBreak*/,/*cHeaderAlign*/,/*lCellBreak*/,/*nColSpace*/,.F./*lAutoSize*/,/*nClrBack*/,/*nClrFore*/)
	TRCell():New(oSection2,'B1_DESC' 	,'SB1',STR0059   ,PesqPict('SB1','B1_DESC')   ,TamSX3('B1_DESC')[1],/*lPixel*/,/*{|| code-block de impressao }*/,/*cAlign*/,/*lLineBreak*/,/*cHeaderAlign*/,/*lCellBreak*/,/*nColSpace*/,/*lAutoSize*/,/*nClrBack*/,/*nClrFore*/)
	//oSection2:Cell("B1_DESC"):SetLineBreak()
	TRCell():New(oSection2,'Basica' 	,'SD4',"Basica"	 ,PesqPict('SD4','D4_QUANT')  ,TamSX3('D4_QUANT')[1]  ,/*lPixel*/,/*{|| code-block de impressao }*/,/*cAlign*/,/*lLineBreak*/,"right",/*lCellBreak*/,/*nColSpace*/,/*lAutoSize*/,/*nClrBack*/,/*nClrFore*/)
	...
	TRCell():New(oSection2,'Qtd. Utilizada'	,'SD4',"Qtd. Utilizada"   ,'@!'  , 15        ,/*lPixel*/,{|| " ___________________" })
	...
	TRCell():New(oSection3,'H8_QUANT'	,'SH8',/*Titulo*/,/*Picture*/,/*Tamanho*/,/*lPixel*/,/*{|| code-block de impressao }*/)
	...
	//³ Sessao 4 (oSection4)                                         ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	oSection4 := TRSection():New(oSection3,STR0053,{"SG2","SH8","SH1","SH4"},/*dOrdem*/) //"Tempo Rot. Oper."
	...
	TRCell():New(oSection4,'IniAloc'	,'SH8',"Inc.Real"   ,'@!'  , 24        ,/*lPixel*/,{|| " ____/ ____/____ ___:___" })
	TRCell():New(oSection4,'FimAloc'	,'SH8',"Fim Real"   ,'@!'  , 24        ,/*lPixel*/,{|| " ____/ ____/____ ___:___" })
	TRCell():New(oSection4,'QtdeProd'	,'SH8',"Qtd.Prod."   ,/*Picture*/, 06       ,/*lPixel*/,{|| Space(06) })
	TRCell():New(oSection4,'QtdPerda'	,'SH8',"Perd."   ,/*Picture*/,/*Tamanho*/,/*lPixel*/,{|| Space(06) })
	...
	//oSection5 := TRSection():New(oSection3,STR0053,{"SG2","SH8","SH1","SH4"},/*dOrdem*/) //"Tempo Rot. Oper."
	//oSection5:SetLineStyle() //Define a impressao da secao em linha
	//oSection5:SetReadOnly()


	//oReport:box(10,10,50,50)
...
Static Function ReportPrint(oReport, cAliasTop)
	...
	*/
	...
	ElseIf !oReport:oPage:IsLandScape()
		oSection2:Cell("B1_DESC"):SetSize(43) //19
	...
	BeginSql Alias cAliasTop
		...
		SC2.C2_STATUS, SC2.C2_OBS, SC2.C2_TPOP, SB1.B1_TIPO, SB1.B1_QB,
		SC2.R_E_C_N_O_  SC2RECNO

		FROM %table:SC2% SC2
		INNER JOIN %table:SB1% SB1 ON B1_COD = SC2.C2_PRODUTO AND B1_FILIAL = %xFilial:SB1%
		...
		SC2.C2_PRODUTO BETWEEN %Exp:mv_par14% AND %Exp:mv_par15% AND
		SC2.%NotDel%
		%Exp:cWhere03%
	...
	AQRY	:= {}
	AQRY	:= GETLASTQUERY()
	MEMOWRITE('C:\TEMP\SELPCPR004.SQL',AQRY[2])
	...
	While !oReport:Cancel() .And. !(cAliasTop)->(Eof())
		...
		oReport:SetTitle("O R D E M   D E   P R O D U C A O - ("+AllTrim((cAliasTop)->C2_PRODUTO)+")") //   STR0010+(cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD)
		...
		//			nLinBar += 0.2
		//		EndIf
		//		MSBAR3("CODE128",nLinBar,15,Trim(cCode),@oReport:oPrint,Nil,Nil,Nil,Nil ,1.5 ,Nil,Nil,Nil,.F.) //0.5
		//	EndIf
		...
		*/
		...
		//	EndIf
		/*
		oReport:Printtext("Temp.: ____________ ºC",oReport:row(),1900)
		oReport:incrow(30)
		*/	
		...
		For nCntFor := 1 TO Len(aArray)
			...
			If SB1->B1_TIPO # 'MO'
				...
				oSection2:Cell('Basica'):SetValue(((SD4->D4_QUANT*(cAliasTop)->B1_QB)/(cAliasTop)->C2_QUANT))
		...
		oReport:incrow(50)
		oReport:Printtext("Reprocesso: ______________________________________________________________________________________________",oReport:row(),110)
		oReport:incrow(50)
		/*
		oReport:Printtext("__________________________________________________________________________________________________________",oReport:row(),110)
		oReport:incrow(50)
		oReport:Printtext("__________________________________________________________________________________________________________",oReport:row(),110)
		oReport:incrow(50)
		*/
		...
		//	oReport:ThinLine()
		if cRecurso = '000008'
			If oReport:row() > (oReport:PageHeight()-650)
				oReport:EndPage (.T.)
				oReport:incrow(300)
			EndIF
			oTFontTit := TFont():New('Courier new',,-15,.T.)
			oReport:incrow(05)
			nLinFim := oReport:row()+0590
			//oReport:box(oReport:row(),100,nLinFim,2300)
			oReport:incrow(080)
			nRowFst := oReport:row()
			//oReport:line(nRowFst,100,oReport:row(),2300)
			nLinCab := oReport:row() + 50
			nColLin :=  0
			oReport:Say ( nRowFst-75, 1000, "COZIMENTO", oTFontTit)
			oReport:Printtext("Estufa.:",nRowFst+10,110)
			oReport:Printtext("Temperatura Final.:",nRowFst+10,1050)
			oReport:Printtext("Gaiola",nLinCab+10, 110)
			oReport:Printtext("Peso Inicial",nLinCab+10, 470)
			oReport:Printtext("Peso Final",nLinCab+10, 930)
			oReport:Printtext("%",nLinCab+10, 1390)
			oReport:Printtext("C/NC",nLinCab+10, 1850)
			oReport:Printtext("Operador Coz.: __________________________________",nLinFim + 30,110)
			oReport:Printtext("Operador Resfr.: ___________________________________",nLinFim + 30,1250)
			oReport:Printtext("Obs.: _________________________________________________________________________________",nLinFim + 100,110)
			//Colunas da Grade
			For i := 1 to 4
				nColLin := nColLin + 460
				oReport:line(nLinCab,nColLin,nLinFim,nColLin)
			next i
			//Linhas da Grade
			For i := 1 to 10
				oReport:line(nLinCab,100,nLinCab,2300)
				nLinCab := nLinCab+50
			next i
			oReport:incrow(100)
			oReport:ThinLine()
		...
		While SG2->(!Eof()) .And. Eval(&cSeekWhile)
			...
			If lSH8
				While SH8->(!Eof()) .And. SH8->(H8_FILIAL+H8_OP+H8_OPER) == xFilial("SH8")+(cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD)+SG2->G2_OPERAC
					...
					oSection3:Cell('H8_QUANT'):SetValue(aSC2Sld(cAliasTop))
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³ PCPR004  ³ Autor ³ Paulo Boschetti       ³ Data ³ 07.07.92 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ Ordens de Producao                                         ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Sintaxe e ³ PCPR004(void)                                              ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Uso      ³ Generico                                                   ³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
*/
User Function xMATR820R3()
